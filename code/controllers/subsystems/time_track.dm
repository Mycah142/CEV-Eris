...
/datum/controller/subsystem/time_track/fire()
	...
	log_debug(
		list(
			...
			SSzas.cost_pipenets,
			SSzas.cost_atmos_machinery,
			SSzas.cost_tiles_curr,
			SSzas.cost_tiles_def,
			SSzas.cost_edges,
			SSzas.cost_fire_zones,
			SSzas.cost_hotspots,
			SSzas.cost_zones,
			length(SSzas.tiles_to_update),
			length(SSzas.deferred_tiles),
			length(SSzas.active_edges),
			length(SSzas.active_fire_zones),
			length(SSzas.active_hotspots),
			length(SSzas.zones_to_update),
			length(SSzas.networks),
		) + send_maps_values
	)
